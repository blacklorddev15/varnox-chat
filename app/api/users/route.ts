...
import { getSettings, listUsers, searchUsers } from '@/lib/db';
...
export async function GET(req: Request) {
  return handle(async () => {
    ...
    // No term means "who is here", not "nobody" — the panel opens on this.
    const [found, mine] = await Promise.all([
      q.trim().length < 1 ? listUsers(me.id) : searchUsers(q, me.id),
      getSettings(me.id),
    ]);
