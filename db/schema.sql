...
-- ── owner actions ───────────────────────────────────────────────────────────
--
-- Who suspended, reinstated, deleted or blocked whom, and why.
--
-- Nothing recorded this before, which was fine only for as long as there was exactly one owner
-- and that owner was never compromised. The moment either stops being true there is no way to
-- tell a deliberate action from a mistake, and no way to answer "who removed this account"
-- except by asking whoever has the password.
--
-- A row per action rather than a general activity log: this table is only ever written by the
-- admin routes, so every row in it is a decision somebody made about somebody else.
--
-- Keep the semicolon character out of these notes, for the same reason as the columns above.
create table if not exists vx_admin_audit (
  id          bigserial primary key,
...
-- Read newest-first, which is the only way this table is ever read.
create index if not exists vx_admin_audit_at on vx_admin_audit (at desc);

-- ── blocked numbers ─────────────────────────────────────────────────────────
--
-- A suspended account is one SIM away from being a new account, so suspension alone does not
-- stop somebody who intends to come back. This is the part that does.
--
-- The number is stored as phoneKey() writes it, not as typed, so that a block catches the same
-- number however it is entered — with or without a plus, spaced or not.
--
-- There is deliberately no blocked-handles table alongside it. A handle is generated by the app
-- and a display name can be anything, so blocking either stops nothing that a new account does
-- not simply route around. The number is the credential, so the number is what is blocked.
create table if not exists vx_blocked_phones (
...
-- ── reports ─────────────────────────────────────────────────────────────────
--
-- A person telling the owner that something is wrong.
--
-- This is the signal the owner had none of: suspension and deletion are decisions, and until
-- now the only input into them was a suspended account asking for its own suspension to be
-- reviewed. Reports are how a decision gets asked for by somebody who is not the accused.
--
-- `target_name` is snapshotted rather than joined. A report against an account that is later
-- deleted has to stay readable — the name is the whole content of the report, and resolving it
-- live would leave a queue of rows that say nothing.
--
-- `status` is open or closed, and closing records who closed it, because a queue that cannot be
-- emptied stops being read.
create table if not exists vx_reports (
  id          bigserial primary key,
...
-- The queue is read as "open first, newest first", and the duplicate check looks up one
